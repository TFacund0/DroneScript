...
function countNodes(node: unknown): number {
  ...
  const children = (n['misiones'] || n['cmds'] || (n['bloque'] ? [n['bloque']] : []) || (n['cmd'] ? [n['cmd']] : [])) as unknown[];
  return 1 + (Array.isArray(children) ? children.reduce((s: number, c) => s + countNodes(c), 0) : 0);
}
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0d0f14', fontFamily: "'Syne', 'Inter', sans-serif", color: '#e8ecf5', display: 'flex', flexDirection: 'column' }}>
      <link rel="preconnect" href="https://fonts.googleapis.com" />
      <link href="https://fonts.googleapis.com/css2?family=Space+Mono:wght@400;700&family=Syne:wght@400;600;800&display=swap" rel="stylesheet" />

      <Header />

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', flex: 1, height: 'calc(100vh - 61px)', overflow: 'hidden' }}>
        <Editor code={code} setCode={setCode} onAnalyze={analyze} />

        <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <div style={{ display: 'flex', borderBottom: '1px solid #252a38', background: '#13161e' }}>
            {TABS.map(t => (
          ...
          <div style={{
            ...
            {!result && tab !== 'gramatica' && tab !== 'simulacion' && (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: 200, color: '#6b7280', fontSize: 12, gap: 8 }}>
            ...
            {tab === 'simulacion'          && <DroneVisualizer ast={result?.ast ?? null} errors={result?.errors ?? []} />}
          </div>

          <div style={{ padding: '8px 18px', fontSize: 11, fontFamily: "'Space Mono', monospace", borderTop: '1px solid #252a38', background: '#13161e', display: 'flex', alignItems: 'center', gap: 12 }}>
            {!result && <span style={{ color: '#6b7280' }}>— listo para analizar</span>}
            {result && isValid && (
              <>
            ...
            {result && !isValid && (
              <span style={{ color: '#ff3d5a' }}>✗ {errors.length} error{errors.length !== 1 ? 'es' : ''} — línea {errors[0]?.line || '?'}</span>
            )}
          </div>
        </div>
      </div>
