...
type StepKind = 'despegar' | 'mover' | 'aterrizar' | 'sensor' | 'condicional' | 'base' | 'mision';
    ...
    switch (cmd.type) {
      ...
      case 'mover':
        if (cmd.modo === 'base') {
          ...
          steps.push({ kind: 'mover', dx, dy, dist: parseFloat(cmd.distancia ?? '0') || 0, label: `MOVER ${cmd.direccion} ${cmd.distancia}${cmd.unidad || ''}`, mision });
        }
        break;
      case 'aterrizar':
...
const STEP_COLOR: Record<StepKind, string> = {
  despegar: '#00e5a0', mover: '#4d9eff', aterrizar: '#ff3d5a',
  sensor: '#ffd166', condicional: '#ff6b35', base: '#6b7280', mision: '#c084fc',
};
...
export default function DroneVisualizer({ ast, errors }: Props) {
  ...
  useEffect(() => {
    ...
    ctx.strokeStyle = '#252a38'; ctx.lineWidth = 1; ctx.setLineDash([4, 4]);
    ...
    ctx.fillStyle = '#252a38'; ctx.font = 'bold 11px Space Mono, monospace';
    ctx.fillText('N', ORIGIN_X + 6, 16); ctx.fillText('S', ORIGIN_X + 6, CANVAS_H - 6);
    ctx.fillText('E', CANVAS_W - 16, ORIGIN_Y - 6); ctx.fillText('O', 6, ORIGIN_Y - 6);

    ctx.fillStyle = '#00e5a020'; ctx.beginPath(); ctx.arc(ORIGIN_X, ORIGIN_Y, 14, 0, Math.PI * 2); ctx.fill();
    ctx.strokeStyle = '#00e5a040'; ctx.lineWidth = 1; ctx.beginPath(); ctx.arc(ORIGIN_X, ORIGIN_Y, 14, 0, Math.PI * 2); ctx.stroke();
    ctx.fillStyle = '#00e5a0'; ctx.font = 'bold 10px Space Mono, monospace'; ctx.textAlign = 'center';
    ctx.fillText('BASE', ORIGIN_X, ORIGIN_Y + 4); ctx.textAlign = 'left';
    ...
    for (const step of stepsToRender) {
      if (step.kind === 'mover' && step.dx !== undefined && step.dy !== undefined && step.dist !== undefined) {
    ...
    if (path.length > 1) {
      for (let i = 1; i < path.length; i++) {
        ...
        ctx.strokeStyle = '#4d9eff44'; ctx.lineWidth = 1.5; ctx.setLineDash([5, 3]);
    ...
    for (const step of stepsToRender) {
      if (step.kind === 'despegar') {
        ctx.strokeStyle = '#00e5a040'; ctx.lineWidth = 1;
        for (let r = 10; r <= 25; r += 8) { ctx.beginPath(); ctx.arc(px, py, r, 0, Math.PI * 2); ctx.stroke(); }
      }
      if (step.kind === 'aterrizar') { ctx.fillStyle = '#ff3d5a22'; ctx.beginPath(); ctx.arc(px, py, 18, 0, Math.PI * 2); ctx.fill(); }
      if (step.kind === 'sensor') {
        ctx.strokeStyle = '#ffd16644'; ctx.lineWidth = 1;
        for (let r = 15; r <= 35; r += 10) { ctx.beginPath(); ctx.arc(px, py, r, 0, Math.PI * 2); ctx.stroke(); }
      }
      if (step.kind === 'condicional') {
        ctx.strokeStyle = '#ff6b3544'; ctx.lineWidth = 1.5; ctx.beginPath();
        ctx.moveTo(px, py - 16); ctx.lineTo(px + 14, py); ctx.lineTo(px, py + 16); ctx.lineTo(px - 14, py); ctx.closePath(); ctx.stroke();
      }
    }

    if (currentStep >= 0) {
      ctx.fillStyle = '#00000055'; ctx.beginPath(); ctx.ellipse(px + 2, py + 3, 12, 6, 0, 0, Math.PI * 2); ctx.fill();
      const arms: [number, number][] = [[1,1],[-1,1],[1,-1],[-1,-1]];
      ctx.strokeStyle = '#252a38'; ctx.lineWidth = 2;
      for (const [ax, ay] of arms) { ctx.beginPath(); ctx.moveTo(px, py); ctx.lineTo(px + ax * 12, py + ay * 12); ctx.stroke(); }
      ctx.fillStyle = '#4d9eff55';
      for (const [ax, ay] of arms) {
        ctx.beginPath(); ctx.arc(px + ax * 12, py + ay * 12, 7, 0, Math.PI * 2); ctx.fill();
        ctx.strokeStyle = '#4d9eff'; ctx.lineWidth = 1;
        ctx.beginPath(); ctx.arc(px + ax * 12, py + ay * 12, 7, 0, Math.PI * 2); ctx.stroke();
      }
      ctx.fillStyle = '#13161e'; ctx.beginPath(); ctx.arc(px, py, 9, 0, Math.PI * 2); ctx.fill();
      ctx.strokeStyle = '#00e5a0'; ctx.lineWidth = 1.5; ctx.beginPath(); ctx.arc(px, py, 9, 0, Math.PI * 2); ctx.stroke();
      ctx.fillStyle = '#00e5a0'; ctx.beginPath(); ctx.arc(px, py, 3, 0, Math.PI * 2); ctx.fill();
      ctx.fillStyle = '#00e5a066'; ctx.beginPath(); ctx.arc(px, py, 6, 0, Math.PI * 2); ctx.fill();
    }
  }, [steps, currentStep]);
  ...
  useEffect(() => {
    if (playing) {
      intervalRef.current = setInterval(() => {
        setCurrentStep(prev => {
          if (prev >= steps.length - 1) { setPlaying(false); if (intervalRef.current) clearInterval(intervalRef.current); return prev; }
          return prev + 1;
        });
      }, 700);
    } else {
  ...
  const btnStyle = (bg: string, color: string, disabled = false): React.CSSProperties => ({
    ...
    fontSize: 12, ...mono, opacity: disabled ? 0.3 : 1, transition: 'all 0.15s',
  });
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#0d0f14' }}>
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        {(hasErrors || noSteps) && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 8, color: '#252a38', fontSize: 12, textAlign: 'center', ...mono }}>
      ...
      {!noSteps && !hasErrors && (
        <div style={{ borderTop: '1px solid #252a38', padding: '10px 16px', background: '#13161e' }}>
          <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: 10 }}>
            {steps.map((step, i) => (
              <button key={i} onClick={() => { setPlaying(false); setCurrentStep(i); }} title={step.label} style={{
          ...
          {currentStep >= 0 && (
            <div style={{ fontSize: 11, ...mono, color: STEP_COLOR[steps[currentStep]?.kind], marginBottom: 8 }}>
              {steps[currentStep]?.label} <span style={{ color: '#6b7280' }}>— paso {currentStep + 1}/{steps.length}</span>
            </div>
          )}
          <div style={{ display: 'flex', gap: 6 }}>
            <button onClick={() => { setPlaying(false); setCurrentStep(-1); }} style={btnStyle('#1a1e28', '#6b7280')}>↩ Reset</button>
            <button onClick={() => setCurrentStep(p => Math.max(-1, p - 1))} disabled={currentStep < 0} style={btnStyle('#1a1e28', '#4d9eff', currentStep < 0)}>◀</button>
            <button onClick={() => setPlaying(p => !p)} style={btnStyle(playing ? '#ff6b3522' : '#00e5a022', playing ? '#ff6b35' : '#00e5a0')}>{playing ? '⏸ Pausar' : '▶ Play'}</button>
            <button onClick={() => { setPlaying(false); setCurrentStep(p => Math.min(steps.length - 1, p + 1)); }} disabled={currentStep >= steps.length - 1} style={btnStyle('#1a1e28', '#4d9eff', currentStep >= steps.length - 1)}>▶</button>
          </div>
        </div>
      )}
    </div>
