interface Rule { lhs: string; rhs: Array<{ kw?: string; t?: string }> }
...
const rules: Rule[] = [
  ...
  { lhs: 'mover',       rhs: [{ kw:'MOVER' }, { t:' (dir NUMBER UNIT? (' }, { kw:'VELOCIDAD' }, { t:' NUMBER)? | ' }, { kw:'BASE' }, { t:')' }] },
  { lhs: 'aterrizar',   rhs: [{ kw:'ATERRIZAR' }] },
  { lhs: 'sensor_cmd',  rhs: [{ kw:'SENSOR' }, { t:' tipo ' }, { kw:'FRECUENCIA' }, { t:' NUMBER UNIT?' }] },
  { lhs: 'condicional', rhs: [{ kw:'SI' }, { t:' IDENT op NUMBER ' }, { kw:'ENTONCES' }, { t:' cmd ' }, { kw:'FIN' }] },
  { lhs: 'dirección',   rhs: [{ t:'norte | sur | este | oeste | noreste | noroeste | sureste | suroeste' }] },
  { lhs: 'tipo_sensor', rhs: [{ t:'temperatura | bateria | altura | velocidad | viento' }] },
  { lhs: 'op',          rhs: [{ t:'< | > | <= | >= | ==' }] },
];
...
const row: React.CSSProperties = { borderBottom: '1px solid #252a38', padding: '6px 0' };

export default function GramaticaPanel() {
  return (
    <div style={{ ...mono, fontSize: 12 }}>
      <div style={{ fontSize: 10, letterSpacing: 2, textTransform: 'uppercase', color: '#6b7280', marginBottom: 12 }}>Gramática formal (BNF)</div>
      {rules.map((r, i) => (
        <div key={i} style={row}>
      ...
      <div style={{ fontSize: 10, letterSpacing: 2, textTransform: 'uppercase', color: '#6b7280', margin: '20px 0 12px' }}>Tokens (expresiones regulares)</div>
      {tokenRules.map((t, i) => (
        <div key={i} style={row}>
