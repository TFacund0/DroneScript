...
function run() {
  ...
  console.log(`${CYAN}   Ejecutor de Pruebas de Integración DroneScript   ${RESET}`);
  console.log(`${CYAN}====================================================${RESET}\n`);

  const casesPath = path.resolve(process.cwd(), "tests/cases.json");
  if (!fs.existsSync(casesPath)) {
    console.error(`${RED}Error: No se encontró el archivo de casos en ${casesPath}${RESET}`);
    process.exit(1);
  }
  ...
  testCases.forEach((tc, idx) => {
    ...
    if (tc.expected.errorMessageContains) {
      const match = errors.some(e =>
        e.message.toLowerCase().includes(tc.expected.errorMessageContains!.toLowerCase())
      );
      if (!match) {
        caseFailed = true;
        failureReasons.push(
    ...
    if (tc.expected.tokenCount !== undefined && lexResult.tokens.length !== tc.expected.tokenCount) {
      caseFailed = true;
      failureReasons.push(
    ...
    if (tc.expected.errorCount === 0) {
      ...
      if (tc.expected.misionNombre !== undefined && mision?.nombre !== tc.expected.misionNombre) {
        caseFailed = true;
        failureReasons.push(
      ...
      if (tc.expected.commandCount !== undefined && mision?.bloque?.cmds?.length !== tc.expected.commandCount) {
        caseFailed = true;
        failureReasons.push(
    ...
    if (caseFailed) {
      ...
      if (errors.length === 0) {
        console.log(`   AST generado con éxito (misión: ${parseResult.ast.misiones[0].nombre}, comandos: ${parseResult.ast.misiones[0].bloque.cmds.length})`);
      } else {
  ...
  console.log(`${CYAN}====================================================${RESET}`);
  console.log(`Resultados: ${GREEN}${passedTests} Pasados${RESET}, ${failedTests > 0 ? RED : GREEN}${failedTests} Fallados${RESET}`);
  console.log(`${CYAN}====================================================${RESET}`);
